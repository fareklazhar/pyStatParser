...
def chomsky_normal_form(tree):
    ...
    n = len(tree)
    if n < 2:
        raise Exception(f"Rule should have at least two items: {str(tree)}")

    if not isinstance(tree[0], basestring):
        raise Exception(f"Root should be a string: {str(tree)}")

    if n == 2:
        # X -> word
        if isinstance(tree[1], list):
            # (1) Normalise single non-terminal on the right
            tree[0] = f"{tree[0]}+{tree[1][0]}"
            tree[1:] = tree[1][1:]
            chomsky_normal_form(tree)
        elif not isinstance(tree[1], basestring):
            raise Exception(f"Terminal should be a string: {str(tree)}")
        ...
        for i in (1, 2):
            if isinstance(tree[i], basestring):
                ...
                raise Exception(f"Non-terminal should be a list: {str(tree)}")
    else:
...
def null_elements_filter(node):
    if isinstance(node, list):
        n = len(node)
        if n >= 2 and n == 2 and '-NONE-' in node[0] or n < 2:
            return False
